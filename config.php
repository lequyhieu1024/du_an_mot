...
function khoahoc() {
    $sql = "SELECT *,khoa_hoc.trang_thai FROM khoa_hoc join danh_muc_khoa_hoc ON khoa_hoc.ma_danh_muc = danh_muc_khoa_hoc.ma_danh_muc";
    $result = pdo_query($sql);
    return $result;
}
function kh_theo_dm(){
    if(isset($_GET['ma_danh_muc'])){
        ...
        $sql = "SELECT * FROM khoa_hoc WHERE ma_danh_muc = '$ma_danh_muc'";
        $result = pdo_query($sql);
        return $result;
    }
}
